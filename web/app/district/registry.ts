...
export function districtGroundHeight(x: number, z: number) {
  ...
  if((ax>=14&&Math.abs(az-10)<=0.125)||(az>=14&&Math.abs(ax-10)<=0.125))return 0.26;
  if((ax>=14&&Math.abs(az-7.25)<=0.14)||(az>=14&&Math.abs(ax-7.25)<=0.14))return 0.2;
  if((ax>=14&&az>=10&&az<=16)||(az>=14&&ax>=10&&ax<=16))return 0.17;
  if(ax>=8&&ax<=16&&az>=8&&az<=16)return 0.13;
  if((ax>=14&&az>=7.35&&az<=9.65)||(az>=14&&ax>=7.35&&ax<=9.65))return 0.03;
  if(Math.abs(az-65)<=2||Math.abs(ax-76)<=2||Math.abs(ax-105)<=2)return 0.025;
  return 0;
}
