...
} from '../world-client/city-operations';
import {
  CityServiceBuildings,
  CITY_SERVICE_COLLIDERS,
} from '../world-client/city-service-buildings';
...
import {
  ...
  requestMallLift,
  type LiftCarrier,
} from '../world-client/mall-circulation';
...
import {
  ...
  AMUSEMENT_PARK,
  type BasketballShot,
} from '../world-client/amusement-park';
import {
  hauntStepAt,
  hauntNextDoor,
  nearBasketballCourt,
  ...
  type HauntStep,
} from '../world-client/amusement-experience';
import {
  PARK_RIDES,
  rideAtStation,
  type RideSession,
} from '../world-client/amusement-rides';
...
export function DistrictClient() {
  ...
  const insideAmusementPark =
    Math.abs(position[0] - AMUSEMENT_PARK.center.x) <=
      AMUSEMENT_PARK.footprint.width / 2 &&
    Math.abs(position[1] - AMUSEMENT_PARK.center.z) <=
      AMUSEMENT_PARK.footprint.depth / 2;
  ...
  const [basketballShot, setBasketballShot] = useState<BasketballShot | null>(
    null,
  );
  ...
  const shootBasketball = () => {
    ...
    setBasketballShot({
      ...
      angle: basketballAngle,
    });
  };
  useEffect(() => {
    const step = walking ? hauntStepAt(position[0], position[1]) : null;
    setHauntStep((before) =>
      before?.house === step?.house && before?.stage === step?.stage
        ? before
        : step,
    );
  ...
  const renderTiles = useMemo(
    () =>
      CITY.tiles.filter(
        (t) =>
          Math.abs(t.cx - housingX) <= 1000 &&
          Math.abs(t.cz - housingZ) <= 1000,
      ),
    [housingX, housingZ],
  );
  ...
  const populationCellX = Math.round(position[0] / 500),
    populationCellZ = Math.round(position[1] / 500);
  useEffect(() => {
    // Population synthesis is CPU-heavy on the Site worker. Let the playable
    // street appear first, then stream residents into the already-running city.
    const timer = window.setTimeout(() => {
      void population.loadArea(populationCellX * 500, populationCellZ * 500);
    }, 2800);
    return () => window.clearTimeout(timer);
  }, [populationCellX, populationCellZ, population.loadArea]);
  ...
  const parkCar = useRef<CarState>({
    ...
    y: districtGroundHeight(
      AMUSEMENT_PARK.center.x + 6,
      AMUSEMENT_PARK.center.z + AMUSEMENT_PARK.entrance.z - 4,
    ),
    yaw: 0,
    speed: 0,
  });
  ...
  const rideLift = (target: number) => {
    ...
    if (
      !liftContains(nearbyLift, position[0], position[1], playerFloor.current)
    ) {
      requestMallLift(nearbyLift, currentMallLevel.y);
      setLiftNotice('已呼梯，请等候开门，走入轿厢后再选目的层');
    } else if (requestMallLift(nearbyLift, target)) {
      liftCarrier.current = {
        active: nearbyLift.phase !== 'idle',
        y: nearbyLift.y,
        carId: nearbyLift.id,
      };
      setLiftNotice(
        `电梯 ${nearbyLift.id} · 前往 ${nearestMallLevel(target).label}`,
      );
    }
    refreshLift((n) => n + 1);
  ...
  const [driving, setDriving] = useState(false),
    ...
    [parkCarReport, setParkCarReport] = useState<CarState>({
      ...parkCar.current,
    });
  ...
  useEffect(() => {
    if (!mounted) return;
    // Avoid parsing every GLB on the same frame. The street becomes playable
    // immediately; progressively richer districts arrive behind it.
    const timers = [
      window.setTimeout(() => setAssetStage(1), 120),
      window.setTimeout(() => setAssetStage(2), 1200),
      window.setTimeout(() => setAssetStage(3), 3200),
      window.setTimeout(() => setAssetStage(4), 5600),
    ];
  ...
  const nearParkCar =
    Math.hypot(
      position[0] - parkCar.current.x,
      position[1] - parkCar.current.z,
    ) < 8 && Math.abs(playerFloor.current - (parkCar.current.y ?? 0)) < 2;
  const nearCar = nearCityCar || nearParkCar;
  const nearRide =
    walking && !driving && !ride
      ? rideAtStation(position[0], position[1])
      : null;
  const beginRide = () => {
      ...
      const pickPark =
        ...
        (!nearCityCar ||
          Math.hypot(
            ...
            Math.hypot(
              position[0] - car.current.x,
              position[1] - car.current.z,
            ));
  ...
  const teleportTo = (x: number, z: number) => {
    ...
    const nearbySolids = [
      ...
      ...CITY.tiles
        .filter(
          (tile) =>
            Math.abs(tile.cx - x) < 1400 && Math.abs(tile.cz - z) < 1400,
        )
        .flatMap((tile) => tile.colliders),
      ...CITY_INFRA.colliders.filter(
        (c) =>
          Math.abs((c.min[0] + c.max[0]) / 2 - x) < 100 &&
          Math.abs((c.min[2] + c.max[2]) / 2 - z) < 100,
      ),
    ].map((c) =>
      c instanceof Box3
        ? c
  ...
  return (
    <main className="district">
      ...
      {walking && hauntStep && (
        <Localized>
          <div className="district-haunt-route" aria-live="polite">
            <strong>
              {hauntStep.house === 'haunt-manor'
                ? 'THE MANOR'
                : 'MIDNIGHT LABORATORY'}
            </strong>
            <span>
              {hauntStep.stage + 1} / 6 · {hauntStep.title}
            </span>
            {nextHauntDoor && (
              <small>
                {nextHauntDoor.side === 'exit'
                  ? '出口在前方'
                  : `下一道门在${nextHauntDoor.side === 'right' ? '右' : '左'}侧`}
                {' · '}
                {Math.round(
                  Math.hypot(
                    nextHauntDoor.x - position[0],
                    nextHauntDoor.z - position[1],
                  ),
                )}{' '}
                米
              </small>
      ...
      {walking && !ride && !driving && (nearRide || nearCar) && (
        <Localized>
          <div className="district-ride-prompt">
            {nearRide ? (
              <>
                <strong>{nearRide.label}</strong>
                <Button onClick={beginRide}>
                  {nearRide.kind === 'tower'
                    ? '按 E 登上跳楼机'
                    : nearRide.kind === 'coaster'
                      ? '按 E 登上过山车'
                      : nearRide.kind === 'carousel'
                        ? '按 E 登上旋转木马'
                        : '按 E 登上旋转茶杯'}
                </Button>
              </>
              ...
              <>
                <strong>{insideAmusementPark ? '园区车' : '汽车'}</strong>
                <Button onClick={toggleCar}>按 E 上车驾驶</Button>
              </>
            )}
          </div>
        </Localized>
      ...
      {ride && (
        <Localized>
          <div className="district-ride-prompt" aria-live="polite">
            <strong>
              {PARK_RIDES.find((entry) => entry.id === ride.id)?.label}
            </strong>
            <small>
              乘坐中 · 鼠标拖动／触控板双指移动视角 · 按 E 随时返回站台
            </small>
            <Button onClick={() => leaveRide(ride.id)}>
              按 E 下车 · Exit ride
            </Button>
          </div>
        </Localized>
      ...
      {scare && (
        <div
          className="district-haunt-scare"
          role="status"
          aria-live="assertive"
        >
      ...
      {walking && nearBasketballCourt(position[0], position[1]) && (
        <div className="district-park-game">
          {!basketballOpen ? (
            <Button onClick={() => setBasketballOpen(true)}>玩投篮挑战</Button>
          ) : (
            <>
              <strong>
                投篮挑战 · {basketballMade} / {basketballAttempts}
              </strong>
              <label>
                出手角度 {basketballAngle}°
                <input
                  type="range"
                  min="36"
                  max="65"
                  value={basketballAngle}
                  onChange={(event) =>
                    setBasketballAngle(Number(event.target.value))
                  }
                />
              </label>
              <label>
                出手速度 {basketballSpeed.toFixed(1)} m/s
                <input
                  type="range"
                  min="15"
                  max="25"
                  step="0.1"
                  value={basketballSpeed}
                  onChange={(event) =>
                    setBasketballSpeed(Number(event.target.value))
                  }
                />
              </label>
              <div>
                <Button onClick={shootBasketball}>投篮</Button>
                <Button onClick={() => setBasketballOpen(false)}>收起</Button>
              </div>
              <small aria-live="polite">
                {basketballResult || '调节角度和力量，让球穿过篮圈。'}
              </small>
            </>
          )}
        </div>
      ...
      <Localized>
        ...
        <nav className="district-tools">
          ...
          <Button
            onClick={() => {
              car.current = { x: 6, z: -68, yaw: 0, speed: 0 };
              setCarReport({ ...car.current });
              setRide(null);
              setActiveCar('city');
              setDriving(true);
              setWalking(true);
          ...
          {walking && insideAmusementPark && (
            <>
              <Button
                onClick={() =>
                  teleportTo(
                    AMUSEMENT_PARK.center.x + 245,
                    AMUSEMENT_PARK.center.z - 280 + 84,
                  )
                }
              >
                古宅鬼屋入口
