...
export const CENSUS_VERSION='ampliworld-residents-30000-2';
export const SOCIAL_JOBS=['酒店前台','销售顾问','客户经理','行政助理','软件工程师','产品经理','设计师','会计','审计员','律师','银行柜员','研究员','医生','护士','药剂师','康复师','教师','幼教老师','图书管理员','实验室技术员','博物馆馆长','策展人','藏品维护员','博物馆讲解员','美术馆讲解员','厨师','服务员','咖啡师','调饮师','烘焙师','外卖员','超市理货员','收银员','店长','手机销售顾问','电子产品维修技师','汽车维修技师','电动车销售顾问','电动车维修技师','摩托车销售顾问','摩托车维修技师','充电运维技师','加油站服务员','理发师','健身教练','保洁员','园林养护员','电工','水管工','维修技师','建筑工人','塔吊司机','水泥搅拌车司机','环卫工人','垃圾车司机','焚烧炉操作员','电力运行员','电网调度员','税务审查员','纳税服务专员','公交司机','出租车司机','网约车司机','货车司机','配送员','仓库管理员','机场地勤','机场安检员','地铁安检员','地铁运营员','物业管家','保安','警员','消防员','社工','照护员','摄影师','音乐教师','创业者','自由职业者','全职家庭照护者','求职者'];
const surnames=['林','陈','周','王','张','李','赵','吴','郑','许','黄','徐','刘','杨','何','宋'];
const names=['晨','宁','安','远','悦','然','清','宇','禾','辰','语','新','明','思','文','嘉'];
function seedRole(index:number){
  ...
  const employed=!child&&age<65&&!['大学生','全职家庭照护者','求职者'].includes(occupation);
  return {family,slot,kind,age,child,occupation,employed};
}
export function citizen(index:number){
  ...
  const {family,slot,kind,age,child,occupation,employed}=seedRole(index);
  ...
  const peerFamily=family%1200<600?family+600:family-600,peerIndex=peerFamily*3+slot;
  if(employed&&peerIndex<CENSUS_SIZE&&seedRole(peerIndex).employed)relations.push({index:peerIndex,type:'同事'});
