...
/** Compatible with old JSON saves; compact durable snapshots for the 3k cohort. */
export async function encodeSnapshot(world:LifeWorld):Promise<string>{
 ...
 const result='gz1:'+btoa(binary);if(result.length>1_900_000)throw new Error('Population snapshot exceeds safe storage size');return result;
}
