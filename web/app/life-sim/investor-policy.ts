...
export type StockSearch={symbol:string;side:'buy'|'sell'|'hold';score:number;uncertainty:number;reason:string};
...
export function searchStockOpportunity(resident:Resident,market:MarketState):StockSearch{
  const style=resident.frugality>.6?'contrarian':resident.consumerPersona?.dimensions.decision_style==='Deliberative'?'deliberative':'momentum';
  const candidates=Object.keys(market.latest.quotes).sort().map(symbol=>{
    ...
    const score=disconfirmed?0:direction*recent/(.02+volatility)*(.65+.35*resident.risk)-resident.stress/300;
    ...
    return {symbol,side:score>.2?'buy' as const:'hold' as const,score,uncertainty:volatility,reason:`${style} scan: recent ${(recent*100).toFixed(1)}%, volatility ${(volatility*100).toFixed(1)}%, stress ${resident.stress.toFixed(0)}`};
  });
