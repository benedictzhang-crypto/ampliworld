...
import { HOUSING_PLAN,HOUSING_INSTANCES,HOUSING_MODELS,housingPoint,housingInstancePoint,housingGroundHeight,housingColliders } from '../app/world-client/housing-registry.ts';
...
for(const p of HOUSING_PLAN.placements){
 ...
 for(const i of HOUSING_INSTANCES.filter(i=>i.parcel===p.id&&(HOUSING_MODELS[i.model].kit==='GC-HOUSING-KIT-001'||i.model===8))){
  const m=HOUSING_MODELS[i.model],door=m.entrance;
  let hit=false,previous;
  for(let z=m.bounds.max[2]+2;z>=door[1]-1.5;z-=.2){const q=housingInstancePoint(i,door[0],z),y=housingGroundHeight(...q,.215)??.035;
