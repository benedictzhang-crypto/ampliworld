...
The park exists as original 3D geometry in the active city. The map selects it and teleports the player to the gate. A dedicated drivable park car waits beside the gate, independently of the CBD car. The map also offers direct arrival at each of the nine ride platforms. At a platform, E (or the on-screen button) boards the corresponding attraction without queuing. All five coaster trains depart from their stations along modeled spur rails, complete a lap and return to the platform; both tower gondolas lift and descend. The carousel horses and teacups rotate as separate client-side meshes, with a follow camera and automatic return to the boarding point. The family rides have sculpted horse silhouettes, alternating saddles, a lighted carousel rim, and teacups with handles and trim; these remain stylized prototype geometry rather than final art. The park floor, haunted-house floors, basketball court and foam-walk ramp/beam have matching height support; 214 structural and haunted-wall colliders are registered. Entering a haunted house reveals its current room and triggers a short 3D apparition at stages three and six. Basketball allows choosing release angle and speed; a visible ball follows ballistic motion and the hoop-height result is scored.

The park geometry and interaction checks live in `npm run check:amusement`; the export contains no pasted building images. The original headless Chrome map-to-entrance test confirmed that the walker reaches `(-7000, 2432)` grounded. Each new ride is also specified by a station coordinate, duration and continuous travel path that can be checked without relying on a screenshot.

## Deliberate limits and next art/gameplay pass

This is a geometric production prototype, not final AAA art. Nine attractions have first-pass playable rides; safety restraints, audio and refined animation still need work. There is intentionally no queue system. The Halloween routes are walkable and have jump-scare triggers, but room-by-room lighting, audio, branching secrets and final creature art need a dedicated pass. NPC leisure schedules are not connected, so resident avatars do not yet play or work in the park. The park also needs higher-quality materials, signage, terrain sculpting and rigorous in-engine walking QA at every attraction.
