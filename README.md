...
**Adaptive resident memory — 23 September:** each resident now persists a bounded experience trace and slowly revises food/leisure preference, family context and market-search style after observed outcomes. Repeated losses reduce future paper position size; routine preferences gradually decay rather than remaining permanently fixed. Existing saves migrate without changing balances or holdings. This is inspectable rule adaptation, not a trained model or an empirical claim about human behavior.

**Wellbeing equilibrium and six-axis view — 23 September:** long-run happiness, short-run mood, stress and simulated mental health now evolve separately. Individual setpoints and fading routine rewards aim to keep the city stable before events; adverse news can lower its average through persistent per-resident pressure. Both the full-city and individual six-axis views include physical health and a liquid-buffer estimate of financial security; the full-city figure is calculated before the client receives its smaller spatial sample. These scores are synthetic, not medical diagnoses or calibrated predictions; public price headlines do not yet reprice every merchant's inventory.

**Crescent Commons — 24 September:** a new 640 × 290 m public sports park southeast of the stadium adds a full-size community football pitch, two basketball courts, hard and red-clay tennis, a bowling hall with 12 visible lanes, and a rooftop tennis court reached by a 54-step stair. Curved footpaths, woodland groups, planting, seating and a roadside gate sit on a checked vacant parcel; its destination is searchable and reachable by map teleport. The original GLB has a semantic collision/walkable-surface manifest and passes `npm run check:commons`. Courts, bowling and sport-specific NPC activities are physical/visual prototypes, not yet complete playable sports systems.

**CBD street-detail pass — 24 September:** finer stone sidewalk joints, continuous curbs, drainage grates, crosswalks and planted pause points accompany a less washed-out metropolitan terrain/sidewalk palette. Four road-facing 40 km/h signs, two blue overhead direction boards, three pedestrian wayfinding posts, four bus shelters and one full-size bus are modeled with registered solid obstacles. Client-rendered lettering matches their 3D frames. The bus is parked; routes, schedules, boarding and service operation are not implemented yet.
